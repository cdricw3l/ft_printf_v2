...
static void	case_managment(va_list *list, char c, int *r)
{
	...
	if (c == 'u')
		ft_putptr_base(va_arg(*list, unsigned int), BASE_D, 1, r);
	if (c == 's')
		ft_putstr_fd(va_arg(*list, char *), 1, r);
	if (c == 'x')
		ft_putptr_base(va_arg(*list, unsigned int), BASE_HEX_LOW, 1, r);
	if (c == 'X')
		ft_putptr_base(va_arg(*list, unsigned int), BASE_HEX_UP, 1, r);
	if (c == 'p')
	{
		(*r) += write(1, "0x", 2);
